...
function Transmission({ id, name }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(`https://0801241705-production.up.railway.app/transmission/delete/${id}`, {
        method: 'GET',
        headers: {
  ...
  const handleEdit = () => {
    console.log(`Modifier la Transmission avec l'id ${id}`);
  };
